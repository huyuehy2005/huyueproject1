...
(function(){
    ...
    var content=document.getElementById('mainLazyImg'),
        floorDivs=utils.getChildren(content,"div"),
            floor=document.getElementById('floorlist'),
        ofloor=document.getElementById('index-floor'),
        floors=utils.getChildren(floor,"li");

    var aLis=[];
    function sort(){
        for(var i=0;i<floors.length;i++){
            if(utils.hasClass(floors[i],'sort')){
                aLis.push(floors[i])
            }
        }
        return aLis
    ...
    sort();
    on(window,'scroll',computedShow);
    function computedShow() {
        var curTop = document.documentElement.scrollTop || document.body.scrollTop;

        if ((curTop + winH / 2) >= floorDivs[0].offsetTop) {
            ofloor.style.display='block';
        } else {
            ofloor.style.display='none';
        }
        for(var i=0;i<floorDivs.length;i++){
            var curOffTop=floorDivs[i].offsetTop;

            /*aLis[i].className='sort fl'*/
            if ((curTop + winH / 2) >= curOffTop) {

                var sib=utils.siblings(aLis[i]);
                for(var j=0;j<sib.length;j++){
                    aLis[j].className='sort fl'
                }
                aLis[i].className='sort fl on'



            /*    aLis[this.index].className='sort fl on'*/
    ...
    for(var i=0;i<aLis.length;i++){
        ...
        aLis[i].onclick=function() {
            ...
            var tarTop=floorDivs[index].offsetTop;
            off(window,'scroll',computedShow);
            move(tarTop);
        }
    }
